...
function modCard(userId, data) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setAuthor({ name: "✦ FATE STAFF CONSOLE • PLAYER PROFILE" })
    .setTitle(display)
    .setDescription(`**@${name}**  •  Roblox ID: \`${userId}\`\n${trim(profile?.description, 500)}\n\n> Pick an action below. Every game-changing action shows a confirmation before it is sent to Roblox.`)
    .addFields(
      ...
      { name: "How to use the buttons", value: "**Ban Player** = permanent restriction • **Temp Ban** = timed restriction • **Unban Player** = remove restriction\n**Give Character** = permanent character access • **Remove Character** = revoke access • **View Gifts** = see character history\n**Add / Remove / Set Coins** = change saved coins • **Refresh Player** = reload current Roblox information", inline: false },
    )
    .setFooter({ text: "Fate Staff Console • Every action is confirmed, logged, and applied live" })
    .setTimestamp();
  if (data?.avatar) embed.setThumbnail(data.avatar);
  return { embeds: [embed], components: [
    new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`twf:ban:${userId}`).setLabel("🛡️ Ban Player").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`twf:tempban:${userId}`).setLabel("⏱️ Temp Ban").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`twf:unban:${userId}`).setLabel("✅ Unban Player").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`twf:history:${userId}`).setLabel("📚 View History").setStyle(ButtonStyle.Secondary)),
    new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`twf:gift:${userId}`).setLabel("🎁 Give Character").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`twf:revoke:${userId}`).setLabel("🚫 Remove Character").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`twf:gifts:${userId}`).setLabel("🎭 View Gifts").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`twf:refresh:${userId}`).setLabel("🔄 Refresh Player").setStyle(ButtonStyle.Secondary)),
    new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`twf:coins_add:${userId}`).setLabel("🪙 Add Coins").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`twf:coins_remove:${userId}`).setLabel("💸 Remove Coins").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`twf:coins_set:${userId}`).setLabel("⚖️ Set Coin Balance").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setLabel("↗ Open Roblox Profile").setStyle(ButtonStyle.Link).setURL(`https://www.roblox.com/users/${userId}/profile`)),
  ] };
}
...
function helpCard() { return new EmbedBuilder().setColor(0x8b5cf6).setTitle("✦ FATE STAFF CONSOLE • QUICK GUIDE").setDescription("A clean control centre for The Witches Fate. Enter a Roblox username or UserId, then confirm every game-changing action.").addFields({ name: "👤 Start with a player", value: "`/fate user` — opens the complete player panel with plain-language buttons.", inline: false }, { name: "🎭 Characters", value: "`/fate give` • `/fate take` • `/fate chars`", inline: false }, { name: "🪙 Coins & vouchers", value: "`/fate add` • `/fate remove` • `/fate set` • `/fate voucher-create`", inline: false }, { name: "🛡️ Staff setup", value: "`/fate perms` • `/fate access-role` • `/fate access-member` • `/fate log`", inline: false }).setFooter({ text: "Fate Staff Console • actions apply in game within seconds" }).setTimestamp(); }
function statusCard(interaction) { const available = Object.keys(permissionNames).filter((key) => key !== "all" && authorized(interaction, key)).map((key) => permissionNames[key]); const storage = storageStatus(); return new EmbedBuilder().setColor(storage.persistentConfigured ? 0x57f287 : 0xfaa61a).setTitle("✦ FATE STAFF CONSOLE • SYSTEM STATUS").setDescription("The Discord console is online and connected to the Roblox action bridge.").addFields({ name: "Your access", value: botOwner(interaction) ? "👑 Server owner / bot owner — full access" : available.length ? available.join("\n") : "No Fate Console permissions", inline: false }, { name: "Safety", value: "✅ Confirmations • ✅ Audit logging • ✅ One-time live game actions", inline: false }, { name: "Permission storage", value: storage.persistentConfigured ? "✅ Persistent storage configured — role permissions survive restarts." : "⚠️ Temporary local storage — set `TWF_DATA_DIR` to a persistent disk before relying on role permissions.", inline: false }).setTimestamp(); }
function charactersCard() { return new EmbedBuilder().setColor(0x8b5cf6).setTitle(`✦ FATE CHARACTER DIRECTORY • ${TWF_CHARACTERS.length}`).setDescription(TWF_CHARACTERS.map((name) => `• ${name}`).join("\n")).setFooter({ text: "Use /fate give with a Roblox username or UserId. Scarlet Sheila is custom-only." }).setTimestamp(); }
