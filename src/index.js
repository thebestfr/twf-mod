...
const permissionNames = { mod: "🛡️ Mod panel + bans", characters: "🎭 Characters", coins: "🪙 Coins", codes: "🎟️ Coin codes", audit: "📜 Audit log", server: "📡 Live server controls", all: "👑 Everything" };
const permissionForSubcommand = (sub) => ({ mod: "mod", gift: "characters", revoke: "characters", "coins-add": "coins", "coins-remove": "coins", "coins-set": "coins", "code-create": "codes", "code-disable": "codes", audit: "audit", "character-list": "characters", announce: "server", pvp: "server", "character-menu": "server", warn: "server", heal: "server", ban: "mod", tempban: "mod", unban: "mod" })[sub] || "mod";
...
function helpCard() { return new EmbedBuilder().setColor(0x5865f2).setTitle("✨ TWF Mod • Staff Guide").setDescription("A live Discord control centre for The Witches Fate. Every game-changing action is logged and applied by the game bridge.").addFields({ name: "🛡️ Player moderation", value: "`/twf mod` • `/twf ban` • `/twf tempban` • `/twf unban`", inline: false }, { name: "🎭 Character access", value: "`/twf gift` • `/twf revoke` • `/twf character-list`", inline: false }, { name: "🪙 Economy", value: "`/twf coins-add` • `/twf coins-remove` • `/twf coins-set` • coin-code commands", inline: false }, { name: "📡 Live server tools", value: "`/twf announce` • `/twf pvp` • `/twf character-menu` • `/twf warn` • `/twf heal`", inline: false }, { name: "⚙️ Staff setup", value: "`/twf permissions-list` • `/twf permissions-role` • `/twf permissions-user` • `/twf audit`", inline: false }).setFooter({ text: "TWF Mod • actions apply in game within seconds" }).setTimestamp(); }
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (["announce", "pvp", "character-menu"].includes(sub)) {
    const action = sub === "announce" ? "announce" : sub === "pvp" ? (interaction.options.getBoolean("enabled", true) ? "pvp_enable" : "pvp_disable") : (interaction.options.getBoolean("locked", true) ? "character_menu_lock" : "character_menu_unlock");
    const reason = sub === "announce" ? interaction.options.getString("message", true).trim() : "";
    enqueue({ action, userId: "0", reason, issuedBy: staff(interaction) });
    const label = sub === "announce" ? `Announcement sent: ${reason}` : sub === "pvp" ? (action === "pvp_enable" ? "PvP enabled" : "PvP disabled") : (action === "character_menu_lock" ? "Character menu locked" : "Character menu unlocked");
    addHistory("system", { action: label, issuedBy: staff(interaction) }); audit(interaction, action, "all servers", reason || label);
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x57f287).setTitle("📡 TWF Live Server Action Queued").setDescription(`**${label}**\n\nThe active TWF servers will update within a few seconds.`).setFooter({ text: `TWF Mod • ${staff(interaction)}` }).setTimestamp()], ephemeral: true });
  }
  ...
  if (["ban", "tempban", "unban"].includes(sub)) {
    const seconds = sub === "tempban" ? parseDuration(interaction.options.getString("duration", true)) : 0;
    if (sub === "tempban" && !seconds) return interaction.reply({ content: "Use a duration like `30m`, `1h`, `7d`, or `1w`.", ephemeral: true });
    const result = moderate(sub, userId, interaction.options.getString("reason")?.trim() || (sub === "unban" ? "Unbanned by TWF staff" : "No reason provided"), seconds, staff(interaction));
    if (result.error) return interaction.reply({ content: result.error, ephemeral: true });
    audit(interaction, sub, userId, interaction.options.getString("reason")?.trim() || "");
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(sub === "unban" ? 0x57f287 : 0xed4245).setTitle(sub === "unban" ? "🔓 TWF Ban Removed" : sub === "ban" ? "🔨 TWF Permanent Ban Applied" : "⏳ TWF Temporary Ban Applied").setDescription(`Roblox user \`${userId}\` has been ${sub === "unban" ? "unbanned" : "restricted"}${seconds ? ` for **${durationText(seconds)}**` : ""}.`).addFields({ name: "Staff note", value: interaction.options.getString("reason")?.trim() || "No note provided." }).setFooter({ text: `TWF Mod • ${staff(interaction)}` }).setTimestamp()], ephemeral: true });
  }
  if (["warn", "heal"].includes(sub)) {
    const reason = sub === "warn" ? interaction.options.getString("message", true).trim() : "";
    enqueue({ action: sub, userId, reason, issuedBy: staff(interaction) });
    addHistory(userId, { action: sub === "warn" ? "Staff warning sent" : "Full heal requested", reason, issuedBy: staff(interaction) }); audit(interaction, sub, userId, reason);
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(sub === "warn" ? 0xf1c40f : 0x57f287).setTitle(sub === "warn" ? "⚠️ Staff Warning Queued" : "💚 Full Heal Queued").setDescription(sub === "warn" ? `Warning sent to Roblox user \`${userId}\` when they are online.` : `Roblox user \`${userId}\` will be fully healed if they are currently online.`).addFields({ name: "Message", value: reason || "No message needed." }).setFooter({ text: `TWF Mod • ${staff(interaction)}` }).setTimestamp()], ephemeral: true });
  }
